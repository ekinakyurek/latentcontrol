...
WORKDIR /opt/apex
ARG APEX_VERSION_TAG
ARG APEX_URL=https://github.com/NVIDIA/apex.git

RUN git clone --recursive --jobs 0 ${APEX_URL} /opt/apex && \
    if [ -n ${APEX_VERSION_TAG} ]; then \
        git checkout ${APEX_VERSION_TAG} && \
        git submodule sync && \
