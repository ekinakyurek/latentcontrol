...
flags.DEFINE_string('exp_folder', default=None,
                    help='main experiment directory')

flags.DEFINE_string('gpus_to_use', default=None,
                    help='coma seperated gpu ids')
...
def assign_to_gpu(gpus, file):
    logging.info(f"waiting for empty gpu"
                 f" for {file}")
    while True:
...
def main(_):
    train_cmd = (f"export PYTHONHASHSEED=0;"
                 f"python -u scripts/train_model.py --disable_tqdm ")
                ...
                for step_index, steps in enumerate((10, 20)):
                    ...
                    for learning_rate in (0.001, 0.0001):
                        ...
                        if train_type == "finetune":
                            local_exp_folder = os.path.join(FLAGS.exp_folder,
                                                            f"seed_{seed}",
                                                            train_type,
                                                            backbone,
                                                            f"lr_{learning_rate}")
                        ...
                        output_file = os.path.join(local_exp_folder,
                                                   "checkpoints/",
                                                   "iter-15-test.tsv")
                        gpu = assign_to_gpu(gpus, output_file)
                        gpu_str = f"export CUDA_VISIBLE_DEVICES={gpu};"
                        log_str = f" > {local_exp_folder}/log.txt 2> {local_exp_folder}/err.txt "
