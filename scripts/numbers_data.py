...
class ArithmethicDataset(Dataset):
            ...
            for e1 in range(max_digits):
                for e2 in range(max_digits):
                    for _ in range(N_per_digit * e1 * e2):
                        x1 = ArithmethicDataset.random_with_n_digits(e1+1)
                        x2 = ArithmethicDataset.random_with_n_digits(e2+1)
    ...
    def __getitem__(self, idx):
        ...
        input = f"{ex.x1} {ex.op} {ex.x2} equals "
        output = f"{ex.y} ."
        return input, output
...
if __name__ == "__main__":
    ...
    for input_ds, attention_mask in train_loader:
        pdb.set_trace()
