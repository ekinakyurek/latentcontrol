...
class GPTPromptCoderMixin:
    ...
    def initialize_coder(
        ...
        for step in range(self.n_steps+1):
            init_weight_value = torch.eye(self.config.n_embd) +\
                                torch.rand(self.config.n_embd,
                                           self.config.n_embd) * random_range

            if initialize_from_vocab:
                init_bias_value = self.transformer.wte.weight[:n_steps]\
                            .clone().detach().mean(dim=0, keepdims=True)
            else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        if input_ids is not None and input_lengths is not None:
        ...
        #             self._extend_attention_mask(input_attention_mask, n_steps=1)
        ...
        #         latent_embed = self.transformer.wte(lm_ids)
